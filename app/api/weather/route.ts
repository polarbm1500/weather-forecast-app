...
// ============================================================================
// サーバー側でだけ動く API ルート。
// ブラウザは /api/weather?city=東京 を叩き、このファイルが OpenWeatherMap を
// 代理で呼ぶ。API キーはここ（サーバー）でしか読まないので、ブラウザの
// DevTools からキーが見えることはない。
// ============================================================================
...
async function owmFetch<T>(
  ...
  if (!res.ok) {
    if (res.status === 401) {
      throw new OwmError(
        "API キーが無効です。.env.local の OPENWEATHER_API_KEY を確認してください（発行直後は有効になるまで最大2時間ほどかかります）。",
        502,
      );
    }
    if (res.status === 404) {
...
export async function GET(request: Request) {
  ...
  if (!apiKey) {
    return NextResponse.json(
      {
        error:
          "OPENWEATHER_API_KEY が設定されていません。.env.local に追記して開発サーバーを再起動してください。",
      },
      { status: 500 },
    );
  }
  ...
  try {
    ...
    if (latParam !== null && lonParam !== null) {
      ...
      // 都市名から引く場合
      place = await geocode(city, apiKey);
      lat = place.lat;
      lon = place.lon;
    } else {
      return NextResponse.json(
