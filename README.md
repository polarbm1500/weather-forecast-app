...
- **カレンダー**: 月表示のカレンダーで、予報がある日（今日から5日先まで）を選んで切り替え
- **3時間ごとの詳細**: 選んだ日の時間帯別の気温・降水確率・湿度・風速
...
│  └ globals.css
├ components/
│  ├ SearchBar.tsx         … 都市検索・現在地ボタン
│  ├ CurrentCard.tsx       … 現在の天気カード
│  ├ Calendar.tsx          … 月表示カレンダー（日付を選ぶ）
│  ├ DayDetail.tsx         … 選択日の詳細（3時間ごと）
│  └ WeatherIcon.tsx
├ lib/
│  ├ types.ts              … アプリ共通の型
│  └ theme.ts              … 天気に応じた背景色
